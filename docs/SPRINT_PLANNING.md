...
| ID | Tâche | Prompt Claude Code | Priorité |
|---|---|---|---|
| S1-08 | Script scraping / import sentiers | `"Écris un script Node.js qui scrape les données des sentiers depuis randopitons.re (nom, distance, dénivelé, difficulté, région, description, coordonnées GPS départ) et les insère dans la table trails Supabase via l'API REST."` | 🔴 |
