...
**Document de System Design | V3.0 | Mars 2026 — Mis a jour le 27/03/2026**
...
- Budget maitrise → Mapbox (token gratuit), Supabase free tier, Expo
- Données sentiers : 710 sentiers issus de donnees terrain avec GPS reels
...
| Écosystème cartographie | @rnmapbox/maps v10, react-native-maps | flutter_map (moins mature) |
| Librairies offline maps | Mapbox GL Native ✅ | Moins d'options robustes |
...
### Cartographie — Mapbox GL (@rnmapbox/maps v10)

> **Note** : Le projet a migré de MapLibre vers Mapbox pendant le Sprint 2.
> Voir project/DECISIONS.md D002 pour la justification.

```
@rnmapbox/maps v10
├── 4 styles natifs : Outdoor, Satellite, Light, Dark
├── Clustering natif pour les marqueurs
├── Support offline (cache tuiles Mapbox)
├── Rendu vectoriel performant
└── Compatible iOS + Android
```

**Cartes offline : PAS ENCORE FONCTIONNEL**
- Le téléchargement de tuiles .pmtiles nécessite un serveur Linux (non disponible)
- Mapbox propose du cache natif qui pourrait être une alternative
- Documenté dans project/TASKS.md comme feature à implémenter
...
GET  /api/trails/:slug/weather
     ...
     → Appel Open-Meteo API (gratuit, sans clé, cache 30min)
```
...
> **Décision D004** : Open-Meteo choisi à la place de Météo-Concept (500 appels/jour limité)

```
Endpoint utilisé :
  https://api.open-meteo.com/v1/forecast

Données récupérées par sentier (coordonnées GPS du départ) :
  ...
  - Sunrise / Sunset
  - Visibilité
  - Code météo WMO (converti en icônes et descriptions)

Cache : 30 minutes par sentier
Coût : Gratuit, illimité, pas de clé API nécessaire
Implémentation : useWeather.ts + WeatherWidget.tsx
```
...
7. SQLite local mis à jour : { trail_id, path, downloaded_at, size_mb }
         │
8. Mapbox configuré pour lire depuis le fichier local
```
...
Source données :
  ...
  pmtiles convert → Emballe en fichier .pmtiles unique
         │
  Upload sur CDN (à définir — pas encore implémenté)
         │
...
Algorithme de validation :
  ...
  3. Si distance < 100m ET couverture > 80% du sentier → Validé auto
...
```javascript
// Exemple Mapbox Layer pour la gamification (IslandProgressMap.tsx)
map.addLayer({
  id: 'zones-progress',
  type: 'fill',
...
|---|---|---|---|
| Framework mobile | React Native | Flutter | Écosystème carto plus mature, team JS |
| Backend | Supabase | NestJS custom | Time to market, PostGIS inclus, moins de DevOps |
| Cartes | Mapbox GL (@rnmapbox/maps v10) | MapLibre | Meilleur support RN, 4 styles natifs (voir D002) |
