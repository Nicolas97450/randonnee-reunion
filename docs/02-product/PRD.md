...
> Version 3.0 | 20 mars 2026 | Statut : Sprint 4 en cours — Social, GPS avance, gamification fog of war, guidage vocal

---

## Résumé exécutif

**Randonnee Reunion** est une application mobile (iOS & Android) qui centralise toutes les informations necessaires au randonneur sur l'ile de La Reunion : referencement complet des 710 sentiers avec traces GPS reelles, carte interactive Mapbox (4 styles : Outdoor/Satellite/Light/Dark), profil d'elevation SVG, meteo montagne enrichie (UV, rafales, visibilite) via Open-Meteo, etat des sentiers via l'ONF, GPS integre avec navigation enrichie (guidage vocal TTS, boussole magnetometre, tracking background crash-safe, compression Douglas-Peucker) et routing pieton OSRM, systeme de gamification avancee (fog of war 3 modes, 18 zones, validation GPS auto a 80%, badges medailles SVG, leaderboard), fonctionnalites sociales completes (messagerie privee DM, notifications in-app 6 types Realtime, amis, feed communaute, sorties de groupe avec chat temps reel, profil public/prive), et SOS urgence 112.
...
| Randonnees hors de La Reunion | Focus geographique volontaire pour la V1 |
| Reservation d'hebergements ou de guides | Hors du perimetre produit core |
...
- [x] Base de données complète : 710 sentiers issus de donnees terrain avec GPS reels
...
**Cartes & Navigation**
- [x] Carte Mapbox interactive (4 styles : Outdoor/Satellite/Light/Dark, clustering)
...
- [x] Guidage vocal TTS (expo-speech, instructions en navigation)
- [x] Boussole magnetometre (expo-sensors, orientation temps reel)
- [x] GPS tracking background (expo-task-manager, ecran actif via expo-keep-awake)
- [x] GPS crash-safe (sauvegarde AsyncStorage, reprise apres crash)
- [x] Distance Haversine precise + lissage altitude
...
- [x] Alerte hors-sentier (100m, vibration, banniere rouge)
- [x] Auto-recentrage carte sur la position utilisateur
- [x] Suggestions sentiers proches sur la carte
- [x] Resume post-hike (HikeSummaryScreen : pace, alt max/min, D-, profil elevation, partage)
...
**Gamification**
- [x] Carte interactive de l'ile avec 18 zones geographiques
- [x] Fog of war : 3 modes de visualisation, decouverte progressive
...
- [x] Compteur de sentiers realises / total
- [x] Badges medailles ronds (SVG) + anneaux de progression par zone (SVG)
- [x] 18 badges (distance, denivele, regions, social, communaute, defis personnels)
- [x] Leaderboard (RPC Supabase, LeaderboardScreen)
- [x] Defis communautaires (ChallengesScreen, useCommunityChallenge)
...
- [x] Onglet Social dedie (SocialStack : inbox, recherche, amis)
...
- [x] Guidage vocal sur sentier — IMPLEMENTE (expo-speech TTS, useVoiceGuidance)
- Integration avec appareils connectes (Garmin, Apple Watch)
...
| Q2 | ~~Quelle source de donnees pour les fiches sentiers ?~~ | Data / Engineering | ~~Bloquant~~ | RESOLU — 710 sentiers issus de donnees terrain |
| Q3 | ~~Quelle librairie cartographique ?~~ | Engineering | ~~Haute~~ | RESOLU — Mapbox GL (@rnmapbox/maps v10, 4 styles) |
...
| Q6 | Contraintes legales liees aux donnees geographiques (IGN) ? | Legal | Moyenne | OUVERT — donnees geographiques publiques, pas IGN |
...
| Cartographie | Mapbox GL (@rnmapbox/maps v10, 4 styles Outdoor/Satellite/Light/Dark, clustering) | Mapbox |
| Traces sentiers | GeoJSON LineString issus de donnees terrain (710/710) | Supabase + JSON bundle local |
...
| Navigation | React Navigation v7 (bottom tabs + native stacks, SocialStack) | 5 onglets |
...
*Document redige le 16 mars 2026 — Mis a jour le 20 mars 2026 (v3.0 — Sprint 4 : guidage vocal, DM, notifications Realtime, fog of war, GPS background crash-safe, boussole, Mapbox 4 styles, leaderboard, securite)*
